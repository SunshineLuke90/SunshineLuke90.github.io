        ...
        (async function init() {
            ...
            try {
                ...
                async function registerAndPrefetch() {
                    if (!('serviceWorker' in navigator) || !('caches' in window)) { console.debug('ServiceWorker or Cache API not available'); return; }
                    try { const swUrl = new URL('sw-radar.js', location.href).href; await navigator.serviceWorker.register(swUrl, { scope: './' }); console.debug('Service worker registered:', swUrl); } catch (e) { console.debug('Service worker registration failed:', e); }
                    try { await prefetchFrames(framesRef.current); } catch (e) { console.debug('Initial prefetch failed:', e); }
                }
